    ...
    return accountData.filter((user) => {
      const isFoundNameOrEmail =
        ...
        (user?.firstName + user?.lastName)
          .toLowerCase()
          .search(trim(filters.searchKey.toLowerCase())) >= 0;
      const isFoundRole = filters.role === "Tất cả" ? true : user.roleId === role;
      const isFoundBanded =
  ...
  return (
    <Container maxWidth="lg" fixed sx={{ mb: 3 }}>
      <Stack alignItems="center" spacing={8} sx={{ marginTop: "38px" }}>
        <Typography variant="h3">DANH SÁCH QUẢN LÝ</Typography>
        <SearchUserListForm onSearch={(data) => setFilters(data)} />
        <Box>
          <AddButton desc="Thêm người dùng" url={`${pages.addAccountPath}`} sx={{mt: -6}}/>
          <DataGridTable
            columns={columns}
            rows={rows}
            rowHeight={70}
